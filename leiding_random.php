        	               ...
        	               if ($groeps_id) {
							   ...
							   <tr>
								   ...
								   foreach ($groeps_id as $groeps_id){
										//haal data op
										$user = get_userdata($groeps_id);
										// email = voornaam.achternaam@chiroschelle.be
										//$email = strtolower($user->first_name . "." . $user->last_name . "@chiroschelle.be");
										//$email = str_replace(" ", "", $email);
									?>
									<tr>
										<td class="tb_avatar"><a class="" href="<?php bloginfo("url") ?>/author/<?php echo $user->user_login ?>"><?php echo get_avatar( $groeps_id, $size = '100'); ?></a></td>
										<td class="tb_info hoogte"><h3><a class="leidingnaam" href="<?php bloginfo("url") ?>/author/<?php echo $user->user_login ?>"><?php echo $user->first_name . " " . $user->last_name; ?></a></h3>
											<p>
												<?php echo $user->straat . " ". $user->nr; ?><br/>
                                                <?php echo $user->postcode . " ". $user->gemeente; ?><br /><br />
												<?php echo $user->telefoon; ?><br />
												<a class="contacteerleiding" href="<?php echo get_bloginfo('url') . '/contact/?uid=' . $user->ID ?>">Contacteer <?php echo $user->first_name; ?></a>
										   </p>
									   </td>
									</tr>
								   <?php } ?>
							...
							foreach ($l as $id){
								$user = get_userdata($id);
                                 ...
                                 $email = str_replace(" ", "", $email);
									...
									/* rechts uitgelijnd */
									 ?>
                                        <tr>
											...
											<td class="tb_avatarrechts"><a class="" href="<?php bloginfo("url") ?>/author/<?php echo $user->user_login ?>"><?php echo get_avatar( $id, $size = '100' ); ?></a> </td>
									...
									?>
                                        <tr>
                                            <td><a class="" href="<?php bloginfo("url") ?>/author/<?php echo $user->user_login ?>"><?php echo get_avatar( $id, $size = '100' ); ?></a></td>
                                            <td class="tb_info hoogte"><h3><a class="leidingnaam" href="<?php bloginfo("url") ?>/author/<?php echo $user->user_login ?>"><?php echo $user->first_name . " " . $user->last_name; ?></a></h3>
                                                <p>
												<?php echo $user->straat . " ". $user->nr; ?><br/>
                                                <?php echo $user->postcode . " ". $user->gemeente; ?><br /><br />
...
//opbouw vb
        	               ...
        	               foreach ($vb_id as $vb_id){
        	               	$user = get_userdata($vb_id);
        	               	if ($user->rank == 'leiding') {
        	               	$email = strtolower($user->first_name . "." . $user->last_name . "@chiroschelle.be");
                            $email = str_replace(" ", "", $email);?>
                                <tr>
                                    <td class="tb_avatarleeg"></td>
                                    ...
                                    <td class="tb_avatarrechts"><a class="" href="<?php bloginfo("url") ?>/author/<?php echo $user->user_login ?>"><?php echo get_avatar($vb_id, '100'); ?></a> </td>
                                </tr>
        	               <?php }} ?>
                                <tr>
                                    <td></td>

 ...
 */
//define( 'ABSPATH',  . '/' ); 
define ('WP_CONTENT_DIR', dirname(dirname(dirname(__FILE__)))); // this is the standard set up with wp-content living 3 levels down
...
if( !is_numeric($id) && $id != '') {
	 display_error ('no id specified or id is not number');
}

define ('DIRECTORY_CACHE', AVATARS_DIR.$id);		// set the cache directory to be inside the users avatar folder 

// set $src to be the large image 

if (  file_exists(AVATARS_DIR.$id) ) {
		
		// Open directory
		if ( $av_dir = opendir( AVATARS_DIR.$id ) ) {
			...
			if ( 0 < count( $avatar_files ) ) {
				...
				if( is_array($avatar_files) ):
					foreach( $avatar_files as $key => $value ) {
						if ( strpos ( $value, '-bpfull' )!== false )
							$src = AVATARS_DIR.$id."/". $value;
							}
				endif;
	...
	display_error ('The Id is not valid');
...
function display_error ($errorString = '') {
	...
	echo '<br />Modified Version of TimThumb version : ' . VERSION . '</pre>';
    die ();

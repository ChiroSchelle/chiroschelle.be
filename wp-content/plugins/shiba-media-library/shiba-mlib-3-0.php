...
class Shiba_Media_Library_3_0 {
	
	function init_admin_3_0() {
		global $shiba_mlib;
		...
		// Add expanded search for edit gallery page
		if (isset($_GET['post_type']) && ($_GET['post_type'] == 'gallery') && strpos($_SERVER["REQUEST_URI"], "wp-admin/edit.php") !== FALSE) {
			add_action('restrict_manage_posts', array(&$this,'expanded_search_menu') );	
			// Filter for expanded search - in media library and manage_gallery page
			add_filter('request', array(&$shiba_mlib,'expanded_media_search'));  // parse_request function
			add_filter( 'get_search_query', array(&$shiba_mlib, 'get_search_query') );
		}
...
//		remove_meta_box('tagsdiv-post_tag','gallery','core');
	...
	function add_pages() {
		...
		$shiba_mlib->media_tools = add_submenu_page( $url, 'Media Tools', 'Media Tools', 'administrator', 'shiba_media_tools', array(&$this,'media_tools_page') );
	...
	function manage_gallery_columns($column_name, $id) {
		...
		case 'images':
			...
			$images = NULL;
			$tag_str = $shiba_mlib->get_post_tags_slug($id);
			if ($tag_str) {
				$args = array(
					'tag' => $tag_str,
	...
	function expanded_search_menu() {
		if (isset($_REQUEST['locked']) || isset($_REQUEST['updated']) || isset($_REQUEST['deleted']) || 
			isset($_REQUEST['trashed']) || isset($_REQUEST['untrashed']) ) 
			$search_top = 145; else $search_top = 95;				
		?>
        <style>
...
//		if (!isset($_GET['post_type']) || ($_GET['post_type'] != 'gallery')) return;
	...
	function add_insert_into_post_button($form_fields, $post) {
		...
		if ($calling_post_id && in_array($post_type, array('post','page')))
...
} // end Shiba_Media_Library_3_0 class
endif;

...
class Shiba_Library_Menu {
	...
	.column-images { width: 50px; text-align: center; }
	.column-att_tag { width: 80px; }
	</style>
	...
	function addMediaActions(mediaPlusForm) {
		...
		var ref = document.getElementsByName('_wp_http_referer');
			...
			document.getElementById('mlib_doaction').name="plusaction";		
		}
	}	
	...
	function library_redirect($location, $status) {
		...
		global $shiba_mlib;		
		if (is_object($shiba_mlib->manage_3_0)) {
			if (strpos($location, 'redirect_back=upload.php') !== FALSE )
				$location = $shiba_mlib->manage_3_0->process_upload_messages($location);
		}	
	...
	function library_action() {
		...
		if ( !isset($_GET['plusaction']) || !isset($_GET['action']) || !$_GET['media'] )return;
		...
		<div id="find-posts" class="find-box" style="display:none;">
			...
			<div class="find-box-inside">
				<div class="find-box-search">
					...
					<input type="button" onClick="findGalleryPosts.send();" value="<?php esc_attr_e( 'Search' ); ?>" class="button" /><br />
					<input type="radio" name="find-posts-what" id="find-posts-pages" checked value="gallery" />
					<label for="find-posts-pages"><?php _e( 'Galleries' ); ?></label>
					<input type="radio" name="find-posts-what" id="find-posts-posts" value="post" />
					...
					<?php if (function_exists('init_shiba_bg_image')) :?>
					<input type="radio" name="find-posts-what" id="find-posts-pages" value="background" />
					<label for="find-posts-pages"><?php _e( 'Backgrounds' ); ?></label>
					<?php endif;?>
			...
			<div class="find-box-buttons">
				<input type="button" class="button alignleft" onClick="findGalleryPosts.close();" value="<?php esc_attr_e('Close'); ?>" />
	...
	function library_expanded_menu() {
        ...
        <?php 
            ...
            <?php else : ?>
			<style>
			#shiba-search-box { position:absolute; top:<?php echo $search_top;?>px;right:10px; }
			#shiba-mlib-box { position:relative; top:-25px; }
			.select-action {visibility:hidden;}
			#doaction, #doaction2 {visibility:hidden;}
        ...
        <form id="shiba-mlib-form" action="" method="get">
            ...
            <?php if ($shiba_mlib->wp30) $shiba_mlib->manage_3_0->media_library_date_filter(); ?>

            <div id="poststuff" class="metabox-holder" style="width:400px;" >	
            <?php $tag_meta_box = do_meta_boxes('media_library_page', 'normal', NULL); ?>
            </div>				
 
        	 <?php if (!$shiba_mlib->wp30) $this->find_galleries_div(); ?>

        </form>
        </div>

        <?php if (!$shiba_mlib->wp30) { ?>
        </div> <!-- End div wrap -->
        <?php } ?>

...
class Shiba_Tag_Metabox {
	...
	function post_tags_meta_box($post, $box) {
		global $shiba_mlib;
		if ($shiba_mlib->wp30) { return $this->post_tags_meta_box_3_0($post, $box); }
	...
	<div class="tagsdiv" id="<?php echo $tax_name; ?>">
		...
		<textarea name="<?php echo "tax_input[$tax_name]"; ?>" class="the-tags" id="tax-input[<?php echo $tax_name; ?>]"><?php if (is_object($post)) echo esc_attr(get_terms_to_edit( $post->ID, $tax_name )); ?></textarea></div>
   ...
   <a href="#titlediv" class="tagcloud-link" id="link-<?php echo $tax_name.'-shiba_gallery'; ?>"><?php printf( __('Gallery Tags'), $box['title'] ); ?></a>
	...
	function post_tags_meta_box_3_0($post, $box) {
		$tax_name = esc_attr(substr($box['id'], 8));
		$taxonomy = get_taxonomy($tax_name);

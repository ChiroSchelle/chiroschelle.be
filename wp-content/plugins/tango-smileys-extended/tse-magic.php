...
function tse_switcher( $tse_content ) {
	$tse_output  = '';
	$tse_smileys = get_tse_smileys();
	...
	$tse_content_arr = preg_split( "~(<.*>)~U", $tse_content, -1, PREG_SPLIT_DELIM_CAPTURE );
	for ( $j = 0; $j < count( $tse_content_arr ); $j++ ) {
		$tse_content_split = $tse_content_arr[$j];
		if ( ( strlen( $tse_content_split ) > 0 ) && ( '<' != $tse_content_split{0} ) ) {
			$tse_content_split = preg_replace( $tse_smiley_match, $tse_img_full, $tse_content_split );
		}
		$tse_output .= $tse_content_split;
	}
	return $tse_output;
}

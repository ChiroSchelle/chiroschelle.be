  ...
  if (isset($_REQUEST['code'])){
   $token_url = "https://graph.facebook.com/oauth/access_token?client_id=" . $app_id . "&redirect_uri=http://stubru.chiroschelle.be/wall&client_secret=" . $app_secret . "&code=" . $_REQUEST['code'];
	...
	if (inDb($user, 'wall')==true){
		...
		if ($result){
			echo "Er is een bericht op je wall gepost!";
		}else{
			echo "Er ging iets mis, probeer later nog eens.";
		}
		
	}else{
		?>
        <p> Post op je wall wat je deelnam aan onze wedstrijd, en verdubbel je kansen!. Het enige wat je hoeft te doen is op de knop klikken:</p>
		<?php echo "<a href='https://www.facebook.com/dialog/oauth?client_id=$app_id&redirect_uri=http://stubru.chiroschelle.be/wall&scope=publish_stream,email'><img src='http://stubru.chiroschelle.be/wp-content/themes/stubru/img/btn_verdubbel.png' alt='VERDUBBEL JE KANSEN' /></a>"; ?>
        <p>volgend bericht zal op je wall worden gepost: <br /><img src="http://stubru.chiroschelle.be/wp-content/themes/stubru/img/fb_post.jpg" alt="voorbeeld facebookpost" />
        <?php 
	}
  }
  }else{
...
?>
  <p>De winnaars zullen via e-mail op de hoogte worden gebracht na de fuif.</p>
  ...
  <li>Inkomtickets voor onze fuif (Dankzij Centea-agentschap Andr&eacute; De Clercq)</li>
  <li>Kortingsbons voor Bobbejaandland</li>
  <li>Kortingsbons voor Pizzahunt</li>
  </ul>

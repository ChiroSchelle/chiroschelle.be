...
class Song {
	...
	function readFromDB($ID){
		$str = "SELECT * FROM `cr-fm` WHERE `ID`=" . mysql_real_escape_string($ID);
	...
	function writeToDB(){
		if($this->ID != -1){
			$str = "UPDATE  `c7070chi_ruben`.`cr-fm` SET  `Artiest` =  '" . mysql_real_escape_string($this->artist) .
			 "',`Titel` =  '" . mysql_real_escape_string($this->title) . "',`Stemmen` =  '" . $this->stemmen . "' WHERE  `cr-fm`.`ID` =" . $this->ID . ";";
			 $str = str_replace("\\\\\\", "\\", $str);
			 mysql_query($str) or die(mysql_error());
		}
		else{
			$str = "INSERT INTO  `c7070chi_ruben`.`cr-fm` (`ID` ,`Artiest` ,`Titel` ,`Stemmen`, `afbeelding`) VALUES (NULL ,  '" .
			 mysql_real_escape_string($this->artist)  . "',  '"  . mysql_real_escape_string($this->title) . "',  '" . $this->stemmen . "' , NULL);";
			 mysql_query($str); #or die(mysql_error());
...
class Klassement{
	...
	function stem($artist, $title, $stemmen = 1){
		...
		foreach ($this->top as $song) {
			...
			if(ucwords(strtolower($song->getTitle())) == $title && ucwords(strtolower($song->getArtist())) == $artist){
				//echo "ok";
				$song->stem($stemmen);
				return;
			}
		...
		$song = new Song;
		$song->newSong($title, $artist, $stemmen);
	}
...
class Stem {
	...
	function check($title, $artist){
		$title = ucwords(strtolower(mysql_real_escape_string($title)));
		$artist = ucwords(strtolower(mysql_real_escape_string($artist)));	
		if($this->hasSong1()){
			if($title == str_replace("\\\\\\", "\\", ucwords(strtolower(mysql_real_escape_string($this->song1[1]))))
			&& $artist == str_replace("\\\\\\", "\\", ucwords(strtolower(mysql_real_escape_string($this->song1[0]))))){
				return true;
			}
		}
		if($this->hasSong2()){
			if($title == str_replace("\\\\\\", "\\", ucwords(strtolower(mysql_real_escape_string($this->song2[1])))) 
			&& $artist == str_replace("\\\\\\", "\\", ucwords(strtolower(mysql_real_escape_string($this->song2[0]))))){
				return true;
			}
		}
		if($this->hasSong3()){
			if($title == str_replace("\\\\\\", "\\", ucwords(strtolower(mysql_real_escape_string($this->song3[1])))) 
			&& $artist == str_replace("\\\\\\", "\\", ucwords(strtolower(mysql_real_escape_string($this->song3[0]))))){
				return true;
			}
		}
		return false;
...
function checkmail($mail){
	$str = "SELECT * FROM `cr-fmMail` WHERE `mail`='" . mysql_real_escape_string($mail) . "'";
	$result = mysql_query($str);
	//echo $numbers;
...
function setmail($mail){
	$str = "INSERT INTO  `c7070chi_ruben`.`cr-fmMail` (`mail`) VALUES ('" . mysql_real_escape_string($mail) . "');";
	mysql_query($str) or die(mysql_error());
}
...
function stemVoorlopig($artist1, $title1, $artist2, $title2, $artist3, $title3, $mail){
	...
	$mailMD5 = md5($mail);
	$str = "INSERT INTO `c7070chi_ruben`.`cr-fmWacht` (`ID`, `mail`, `Title1`, `Artist1`, `Title2`, `Artist2`, `Title3`, `Artist3`, `adres`) VALUES (NULL, '" . mysql_real_escape_string($mailMD5) . "', '" . mysql_real_escape_string($title1) . "', '" . mysql_real_escape_string($artist1) . "', '" . mysql_real_escape_string($title2) . "', '" . mysql_real_escape_string($artist2) . "', '" . mysql_real_escape_string($title3) . "', '" . mysql_real_escape_string($artist3) . "', '" . mysql_real_escape_string($mail) . "');";
	$str = str_replace("\\\\\\", "\\", $str);
	if(mysql_query($str)){
		setmail($mail);

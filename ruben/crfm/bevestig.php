				...
				<div class="overzicht">
					<?php
						...
						if(isset($_GET['mail'])){
							...
							if(!$result = mysql_query($str) or (!$result)){
								...
								else if(stem($row['Artist1'], $row['Title1'], $row['Artist2'], $row['Title2'], $row['Artist3'], $row['Title3'], $Klassement)){
									$str = "DELETE FROM `c7070chi_ruben`.`cr-fmWacht` WHERE `cr-fmWacht`.`ID` = " . $row['ID'];
									if(mysql_query($str) === false)
									{
										$file = fopen("log.txt", "a");
										fwrite($file, "Kon " . $ID . "niet verwijderen uit cr-fmWacht");
										fclose($file);
									}
									header("Refresh: 5; url=http://www.chiroschelle.be");
									echo "Uw stem is goed ontvangen.<br>";
									echo str_replace("\\", "","Uw stem ging naar:<ol><li>" . $row['Artist1'] . " - " . $row['Title1'] . "</li>");
									echo str_replace("\\", "","<li>" . $row['Artist2'] . " - " . $row['Title2'] . "</li>");
									echo str_replace("\\", "","<li>" . $row['Artist3'] . " - " . $row['Title3'] . "</li></ol>");
								}
								else {
